...
func _ready() -> void:
	...
	print(">>> ENEMY SETUP DONE skeleton=", _skeleton, " bones=", _bones.size())
...
func _find_skeleton() -> void:
	if _body_root == null:
		print(">>> _body_root null")
		return
	_skeleton = _find_skeleton_recursive(_body_root)
	if _skeleton == null:
		print(">>> NO Skeleton3D found")
		return
	print(">>> Skeleton has ", _skeleton.get_bone_count(), " bones")
	var all_names: Array = []
	for i in range(_skeleton.get_bone_count()):
		all_names.append(_skeleton.get_bone_name(i))
	print(">>> Bones: ", all_names)
	var wanted := ["thigh_l", "thigh_r", "calf_l", "calf_r", "upperarm_l", "upperarm_r", "lowerarm_l", "lowerarm_r"]
	for i in range(_skeleton.get_bone_count()):
		...
		for short in wanted:
			if n == short or n.ends_with("/" + short) or n.ends_with(":" + short):
				_bones[short] = i
				break
	print(">>> Matched bones: ", _bones.keys())
...
func _set_bone_swing(bone_short: String, angle: float) -> void:
	...
	var extra := Basis(SWING_AXIS, angle)
